...
namespace Комиссия
{
    public partial class Parents : MetroFramework.Forms.MetroForm
    {
        main mn;
        int id;
        ...
        Абитуриенты abitF;
        string oldValue;
        public Parents(main mn1, int _id, Абитуриенты _abitF)
        {
            InitializeComponent();
            ...
            con.Open();
            da = new SqlDataAdapter("SELECT * FROM Parents where [id абитуриента] = " + id, con);
            ...
            dataGridView1.ColumnHeadersBorderStyle =
                DataGridViewHeaderBorderStyle.Single;
            ((DataGridViewTextBoxColumn)dataGridView1.Columns["ФИО"]).MaxInputLength = 45;
            ((DataGridViewTextBoxColumn)dataGridView1.Columns["Адрес"]).MaxInputLength = 50;
            ((DataGridViewTextBoxColumn)dataGridView1.Columns["Место работы"]).MaxInputLength = 50;
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString()))
            {
                if (e.ColumnIndex == 6)
                {
                    if (!Regex.IsMatch(dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString(), @"375\d{9}") || string.IsNullOrEmpty(dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString()) || dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString().Length > 12 || dataGridView1[e.ColumnIndex, e.RowIndex].Value.ToString().Length == 0)
                    {
                       
                        MessageBox.Show("Номер введён неверно! Повторите ввод");
